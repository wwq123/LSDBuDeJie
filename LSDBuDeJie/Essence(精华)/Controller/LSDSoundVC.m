...
@interface LSDSoundVC ()

@end

@implementation LSDSoundVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.tableView.backgroundColor = [UIColor blueColor];
    self.tableView.contentInset = UIEdgeInsetsMake(LSD_ContentSet_Top, 0, LSD_ContentSet_bottom, 0);
    [LSDNotificationCenter addObserver:self selector:@selector(sound_repetClickTabBarItemAction:) name:LSDRepetClickTabBarItemNotification object:nil];
     [LSDNotificationCenter addObserver:self selector:@selector(sound_repetClickTitleViewBtnAction:) name:LSDRepetClickTitleViewBtnNotification object:nil];
}

#pragma mark - 监听通知
//监听TabBarItem重复点击
- (void)sound_repetClickTabBarItemAction:(NSNotification *)noti{
...
- (void)sound_repetClickTitleViewBtnAction:(NSNotification *)noti{
    [self sound_repetClickTabBarItemAction:noti];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *identifier = @"LSDSoundVCCell";
